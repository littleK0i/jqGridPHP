...
abstract class jqGrid_DB
{
	...
	public final function __construct(jqGridLoader $loader)
	{
		$this->loader = $loader;
	}
	...
	{
		$tblName = jqGridUtils::checkAlphanum($tblName);
	...
	public function delete($tblName, $cond)
	{
		...
		if(is_numeric($cond))
		{
			$where[] = $this->primary_key . '=' .intval($cond);
		}
		elseif(is_array($cond))
		{
			$cond = $this->cleanArray($cond);
	...
	protected function cleanArray(array $arr)
	{
		...
		foreach($arr as $k => $v)
		{
			$key = jqGridUtils::checkAlphanum($k);
